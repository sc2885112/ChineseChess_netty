...
public class ClientView extends JFrame implements ActionListener{
	...
	JTextField jtfPort=new JTextField("8090");//创建输入端口号的文本框，默认值是9999
    ...
    public String myNickName;  //我的昵称
	public Channel channel; //netty提供的交互通道
	...
	public void openConnection() {
		...
		if (portText > 65535 || portText < 0) {// 端口号不合法，给出错误提示
			JOptionPane.showMessageDialog(this, "端口号只能是0-65535的整数", "错误", JOptionPane.ERROR_MESSAGE);
			return;
		}

        myNickName = jtfNickName.getText().trim();// 获得昵称
		if (myNickName.length() == 0) {// 昵称为空，给出错误提示信息
        ...
        myThread = new Thread(() ->{
            ...
            try {
                ...
                channel.writeAndFlush(ClientMenu.REQUEST_CONNECTION + myNickName);//发送昵称到服务器
                ...
                e.printStackTrace();
                JOptionPane.showMessageDialog(clientView, "连接服务器失败", "错误", JOptionPane.ERROR_MESSAGE);// 连接失败，给出提示信息
            }finally {
	...
	public void launchChallenge() {
        ...
        channel.writeAndFlush(ClientMenu.LAUNCH_CHALLENGE  + myNickName + "-" +  name);	// 向服务器发送挑战信息
	...
	public void extConnection(){
		...
		if(channel != null){
            if(opponentName != null){//如果有正在对弈的对手则带上对手的名字方便服务器回发信息
                channel.writeAndFlush(ClientMenu.USER_EXIT + myNickName + "-" + opponentName);//向服务器发送断开连接的请求
            }else{
                channel.writeAndFlush(ClientMenu.USER_EXIT + myNickName);//向服务器发送断开连接的请求
            }
            channel.close();
        }
	...
	public void admitDefeat() {
        if (JOptionPane.showConfirmDialog(this, "您确定要认输吗？", "认输！",
            ...
            channel.writeAndFlush(ClientMenu.ADMIT_DEFEAT + opponentName);// 向服务器发送认输的请求
	...
	public void sendMessage() {
        ...
        if (message != null && message.length() > 0) {
            ...
            channel.writeAndFlush(ClientMenu.CHAT + message + "-" + myNickName + "-" + opponentName);// 向服务器发送消息
